...
import { useAuth } from "@/context/AuthContext"; // ✅ pour gérer logout si 401
...
export function RecruiterProfileGuard({ children }: RecruiterProfileGuardProps) {
  ...
  const { logout, getValidToken } = useAuth(); // ✅ pour gérer déconnexion si token invalide

  useEffect(() => {
    const checkProfile = async () => {
      try {
        // 🔑 Récupère un token valide (ou null)
